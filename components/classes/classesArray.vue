    ...
    <b-card class="shadow">
      <b-card-header class="d-flex justify-content-between">
        <div class="d-flex align-items-center">
          <b-card-title class="text-lg mr-3">Rechercher un cours :</b-card-title>
          <base-input
        ...
        <div>
          <b-button variant="link" @click="$router.push('/t/add-classes')"><font-awesome-icon icon="plus" class="mr-3"/>Ajouter</b-button>
          <b-dropdown id="dropdown-1" toggle-class="dropdown-toggle-no-caret" dropleft variant="link" text="Dropdown Button" class="m-md-2">
            <template slot="button-content"><font-awesome-icon icon="filter"  class="mr-3"/>filter</template>
          </b-dropdown>
        </div>
      </b-card-header>
      <div style="overflow-x: auto">
        <table class="table">
          ...
          <tr v-for="(classe, key) in resultSearch" :key="key">
            <td>
              <div class="text-sm font-weight-bold">{{ classe.matter.libel }}</div>
              <div class="text-muted">Ajouter le {{ moment(classe.created_at).format('DD-MM-YYYY') }}</div>
            </td>
            <td><div class="text-sm font-weight-bold">{{ classe.teacher.first_name }} {{ classe.teacher.last_name }}</div></td>
            <td>
              <div class="text-sm font-weight-bold">{{ classe.room_nbr }}</div>
              <div class="text-muted">{{ classe.address }}</div>
            </td>
            <td><b-badge>{{ classe.level }}</b-badge></td>
            <td>
...
export default {
  ...
  methods: {
    ...
    nbrStatusPlaceAvailable(participant_nbr, available_nbr) {
      return [
        ...
        {'text-warning': participant_nbr > available_nbr / 2 && participant_nbr < available_nbr - 2},
  ...
  computed: {
    resultSearch() {
      return this.classes.filter(r => r.matter.libel.toLowerCase().match(new RegExp(this.search.toLowerCase() + ".*")))
    }
  },
  async mounted() {
