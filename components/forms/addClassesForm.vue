      ...
      <b-col lg="9" md="7">
        <b-card class="bg-lighter border-0 mb-0">
          <b-card-header class="bg-transparent">
            <div class="font-weight-bold text-lg text-primary mt-2 mb-2">
              Crée votre cours !
            </div>
          </b-card-header>
          <b-card-body class="px-lg-5 py-lg-5">
            <form role="form" @submit.prevent="handleLoginSubmit()">
              <base-search
              ...
              <b-form-group class="mb-3 w-100">
                <label class="form-control-label"> Définissez une date </label>
                <v-date-picker
                  v-model="date"
                  show-caps
                  :columns="$screens({ default: 1, lg: 2 })"
                  :input-props="{ class: 'mb-3 form-control' }"
                />
              </b-form-group>
              <b-form-group class="mb-3 w-100">
                <label class="form-control-label"> Définissez une heure </label>
                <VueTimepicker
                  v-model="time"
                  close-on-complete
                  advanced-keyboard
                  input-class="form-control bg-white w-100 border-0"
                  value=""
...
export default {
  ...
  methods: {
    handleLoginSubmit() {
      this.form.date =
        moment(this.form.date).format("YYYY-MM-DD") + " " + this.time + ":00"
      this.$api.classe
        .create(this.form)
        .then(() => {
          this.$store.dispatch("notifications/addNotification", {
            type: "success",
            message: "Ajout du cours avec success",
          })
